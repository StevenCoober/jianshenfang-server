...
module.exports = class extends Base {
  //  course_topic_type(jianshenfang_course_topic_type) = {
  //    id:int, /*课程主题类型id*/ 
  //    type_name:varchar, /*类型名称, 比如瘦身课*/ 
  ...
  async indexAction() {
    ...
    const model = this.model('course_topic_type');
    let data;
    if (key != '') {
      if (!precision) {
  ...
  async deleteAction() {
    const id = this.post('id');
    let model = this.model('course_topic_type');
