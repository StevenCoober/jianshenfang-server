...
module.exports = class extends Base {
  //  coach_reserve(jianshenfang_coach_reserve) = {
  //    id:int, /*教练预约订单id*/ 
  //    user_id:int, /*预定的会员用户id*/ 
  //    coach_id:int, /*教练id*/ 
  //    skill_id:int, /*预约课程id*/ 
  //    order_state:int, /*订单状态//0未开始1进行中2已完成*/ 
  //    orde_result_state:int, /*预约结果状态*/ 
  //    reverse_btime:datetime, /*预约开始时间 */ 
  //    reverse_etime:datetime, /*预约结束时间 */ 
  ...
  async indexAction() {
    ...
    const model = this.model('coach_reserve');
    let data;
    if (key != '') {
      if (!precision) {
  ...
  async allIndexAction() {
    const page = this.post('page') || 1;
    const size = this.post('size') || 10;
    const key = this.post('key') || '';
    const name = this.post('name') || '';
    const model = this.model('coach_reserve');
    const data = await model.getAllIndex(page, size, key, name);
  ...
  async deleteAction() {
    const id = this.post('id');
    let model = this.model('coach_reserve');
